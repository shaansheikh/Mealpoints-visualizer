...
def index():
	...
	if request.method == 'POST':
		...
		if (u and p and not c):
			...
			globaltree = tree
			data = tree.xpath('//table[@class="display"]//td/text()')
			data = [data[x: x+4] for x in xrange(0, len(data), 4)]
	...
	if scraped == 0:
		...
		info[0] = globaltree.xpath('//div[@class="account-info"]//b/text()')[0]
	...
	try:
		...
		for x in range(len(data)):
			date = data[x][0][:data[x][0].index(' ')] + '/' + str(2016) + data[x][0][data[x][0].index(' '):]

			tempdt = dt.datetime.strptime(date,"%m/%d/%Y %I:%M %p").replace(year=begindt.year)
			if tempdt < begindt:
		...
		for datum in data:
			if float(datum[2]) < 0:
				...
				elif place[:4].lower() == "deli" or place[:5].lower() == "union" or place.lower().find("carlos")>-1:
				...
				elif place.lower().find("wendys")>-1 or place.lower().find("roth")>-1 or place.lower().find("s3 fusion")>-1 or place[:5].lower() == "red m"or place[:9].lower() == "market fr" or place[:9].lower() == "daily dis":
